...
class X {
    ...
    if (this.camera instanceof PerspectiveCamera) {
      ...
      if (this.cameraFov) {
        if (this.cameraMinAspect && this.camera.aspect < this.cameraMinAspect) {
          this.#adjustFov(this.camera, this.cameraMinAspect);
        } else if (
          this.cameraMaxAspect &&
          this.camera.aspect > this.cameraMaxAspect
        ) {
          this.#adjustFov(this.camera, this.cameraMaxAspect);
        } else {
  ...
  #adjustFov(cam: PerspectiveCamera, aspect: number) {
    const tanFov = Math.tan(MathUtils.degToRad(this.cameraFov / 2));
    const newTan = tanFov / (cam.aspect / aspect);
    cam.fov = 2 * MathUtils.radToDeg(Math.atan(newTan));
  }
  ...
  clear() {
    const hasDispose = (x: unknown): x is { dispose: () => void } => {
      return (
        typeof x === "object" &&
        x !== null &&
        typeof (x as { dispose?: unknown }).dispose === "function"
      );
    };
    this.scene.traverse((obj) => {
      if (obj instanceof Mesh) {
        const material: Material | Material[] = obj.material as
          | Material
          | Material[];

        const disposeMaterial = (m: Material) => {
          // Dispose any disposable sub-resources on the material
...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: MeshPhysicalMaterialParameters) {
    ...
    this.onBeforeCompile = (shader: ThreeShader) => {
      Object.assign(
        (shader as { uniforms: Record<string, unknown> }).uniforms,
        this.uniforms
      );
      (shader as { fragmentShader: string }).fragmentShader =
        `
        ...
        ` + (shader as { fragmentShader: string }).fragmentShader;
      (shader as { fragmentShader: string }).fragmentShader = (
        shader as { fragmentShader: string }
      ...
      (shader as { fragmentShader: string }).fragmentShader = (
        shader as { fragmentShader: string }
      ).fragmentShader.replace("#include <lights_fragment_begin>", lightsChunk);
...
type BallpitConfig = Partial<typeof XConfig>;
